<template>
  ...
  >
    <h3 class="padding-bottom-3 padding-top-1 text-center text-size-default">绑定IC卡</h3>
      ...
      >
        <template #input>
         <div class="input-box d-flex align-items-center w-100">
           <input class="van-field__control flex-1" v-model="modelData.card" placeholder="请输入IC卡号" >
           <van-icon name="scan" size="0.55rem" class="text-success font-weight-bold" @click="handleScan" />
         </div>
        </template>
      </van-field>
      <van-field
...
export default defineComponent({
  ...
  setup (props, { emit, root }) {
    ...
    // 提交回调
    const onSubmit = () => {
      const { card, radio } = modelData.value
    ...
    const handleScan = async () => {
      try {
        ...
        if (typeof cardNumber === 'undefined') return root.$toast('请扫描IC卡背面的二维码')
        modelData.value.card = cardNumber
      } catch (e) {
        root.toast('异常错误')
      }
    }
